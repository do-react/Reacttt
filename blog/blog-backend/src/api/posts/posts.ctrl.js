...
export const list = async (ctx) => {
  //query는 문자열이기 때문에 숫자로 변환해 중야 한다.
  //값이 중지지 않았다면 1을 기본으로 사용한다.
  ...
  //특정 사용자가 작성한 포스트만 조회하거나 특정 태그가 잇는 포스트만 조회하는 기능
...
export const remove = async (ctx) => {
  ...
  try {
    ...
    ctx.status = 204; //No Content(송공하기는 했지만 응답할 데이터는 없음)
  } catch (e) {
    ctx.throw(500, e);
  }
};
